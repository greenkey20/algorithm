...
public class Babbling2 {
    // 2023.2.18(토) 17h45 -> 18h v1 의도하는대로 작동x -> 18h20 v2도 의도하는대로 동작x
    ...
    public static int solution(String[] babbling) {
        ...
        for (int i = 0; i < replacedBabbling.length; i++) {
            ...
            for (int j = 0; j < toCheck.length() - 1; j++) {
                ...
                if (Character.isDigit(chLeft)) { // 현재 검사 중인 글자가 숫자이면
                    if (j == toCheck.length() - 1) { // 현재 검사 중인 단어가 숫자 1개로 이루어져 있다면, 또는 이 내부for문에서 이 j 조건까지 왔다면 = 이 단어는 발음할 수 있음
                        answer++;
                        ...
                        if (Character.isDigit(chRight) && chLeft != chRight) { // 다음 글자도 숫자이고, 현재 글자와 다음 글자가 다르다면
                            continue;
                        } else { // 다음 글자가 숫자가 아니거나, 현재 글자와 다음 글자가 같다면 = 이 단어는 발음할 수 없음 -> 다음 단어 검사로 넘어감
                            break;
                        }
                    }
                } else { // 현재 검사 중인 글자가 숫자가 아니면 = 이 단어는 발음할 수 없음 -> 다음 단어 검사로 넘어감
                    break;
                }
            }
        }
